...
import { FlexNode, retranslateEnum, YogaNodeProperties } from "./src"
...
describe("set & get properties", () => {
    ...
    it("it should get the simplified default values", () => {
        properties.forEach((property) =>
            expect(node.getProperty(property)).to.deep.equal(toSimplifiedValue(property, nodeDefaults[property]))
        )
    })
    ...
    it("it should set new values", () => {
        ...
        properties.forEach((property) => expect(node.getProperty(property)).to.equal(testValues[property]))
    })
...
function toSimplifiedValue(name: string, value: typeof nodeDefaults[keyof typeof nodeDefaults]): any {
    let result: typeof nodeDefaults[keyof typeof nodeDefaults] | undefined = value
    if (typeof result === "number" && isNaN(result)) {
        result = undefined
    }
    result = flatten(result)
    return retranslateEnum(name, result)
}
